...
router.get('/lists', async (req, res, next) => {
  ...
  try {
    ...
    let where = { user_id: new ObjectId(user_id) }
    let total = await FollowsModel.countDocuments(where)
...
router.get('/preview', async (req, res, next) => {
  let user_id = req.auth.id
  try {
    let fans_count = await FollowsModel.countDocuments({ user_id: new ObjectId(user_id) })
    let follow_count = await FollowsModel.countDocuments({ fans_id: new ObjectId(user_id) })
    res.send({
      fans_count,
      follow_count,
    })
...
router.post('/is-follow', async (req, res, next) => {
  let fans_id = req.auth.id
  let { user_id } = req.body
  try {
    let count = await FollowsModel.countDocuments({
      user_id: new ObjectId(user_id),
      fans_id: new ObjectId(fans_id),
    })
