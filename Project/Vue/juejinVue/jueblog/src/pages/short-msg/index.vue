...
import { useMessageStore, useShortMsgStore, useUserStore } from '@/stores'
...
const onFilter = (json: Record<string, string>) => {
  ...
  router.push({
    // query: filter.value,
    query: filter.value as unknown as LocationQueryRaw,
  })
  ...
  try {
    ...
    if (fileList.value.length > 0) {
      ...
      if (resimg.code == 200) {
        form.value.images = resimg.data.map(
          (row: any) => 'http://localhost:9000' + row.path,
        )
      }
    }
    store.createMsg(form.value, res => {
      ...
      if (res) {
        ...
        fileList.value = []
        store.getShortmsgs(filter.value as Record<string, string>)
      }
    })
  } catch (error) {
