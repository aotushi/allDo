...
import type { ShortMsgType } from '@/stores/short-msg/type'
const {
...
const { getShortmsgs } = useShortMsgStore()
const route = useRoute()
const curuser = ref<UserType | null>(null)
const tab = ref('article')
const is_follow = ref(false)
const articles = ref<{ meta: IAnyObj; data: unknown[] }>({
  meta: {},
  data: [],
})
const short_msgs = ref<{ meta: IAnyObj; data: ShortMsgType[] }>({
  meta: {},
  data: [],
})
const uid = ref('')
const onChange = (e: MouseEvent) => {
  let dom = e.target
  if (!dom) return
  tab.value = (dom as HTMLElement).dataset.val as string
  getData()
}
const toFollow = () => {
...
const getData = () => {
  if (tab.value == 'article') {
    getArticles({ created_by: uid.value }, res => {
      console.log('res>', res)
      articles.value = res as { meta: IAnyObj; data: unknown[] }
    })
  }
  if (tab.value == 'shortmsg') {
    getShortmsgs({ created_by: uid.value }, res => {
      short_msgs.value = res as { meta: IAnyObj; data: ShortMsgType[] }
    })
  }
}
onMounted(() => {
  ...
  <section class="user-page fx">
    <div class="content-panel">
      ...
      <div class="datainfo panel">
        ...
        <Articles
          v-if="tab == 'article'"
          :articles="articles.data as ArticleType[]"
        />
