...
const toPublish = () => {
  ...
  if (form.value.status && form.value.status == 1) {
    article_store.updateArt(mode.value, form.value, () => {
      loading.value = false
      location.href = '/article/' + mode.value
    })
  } else {
    article_store.publishArt(mode.value, () => {
      loading.value = false
      location.href = '/article/' + mode.value
    })
  }
...
const ctxChange = debounce((key: 'title' | 'content') => {
  ...
  if (mode.value == 'create' && form.value[key]) {
    loading.value = true
    article_store.createArt(form.value, res => {
      mode.value = res._id
...
onMounted(() => {
  ...
  if (tag != 'create') {
    article_store.getArtDetail(tag, data => {
      // article.value = data
      form.value = data
    })
  }
          ...
          <el-form label-width="85px">
            <el-form-item required label="分类：">
              <div
                ...
                v-for="item in article_store.categories"
