...
<template>
  <div class="article-lists">
    <div
      ...
      :key="item._id"
    >
      <div class="art-meta fx" v-if="item.user?.username">
        {{ item.user.username }} <i /> {{ getTimer(item.created_at) }}<i />{{
          article_store.getCateLabel(item.category)
        }}
      </div>
      <div class="artctx-wrapper">
        <div class="info-wrap">
          ...
          <div class="handle fx">
            ...
            <el-dropdown
              v-if="item.user?._id == user_store.user_state.user_info?._id"
              trigger="hover"
