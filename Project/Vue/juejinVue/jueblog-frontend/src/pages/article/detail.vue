...
const directs = ref(
  [] as { top: number; key: string; [key: string]: unknown }[],
)
const actd = ref('')
const is_follow = ref(false)

console.log('detail>user_store.>', user_store.user_state.user_info)
...
const toFollow = () => {
  ...
  user_store.toggleFollow({ user_id }, res => {
    console.log(res)
...
const toPraiseOrStart = (type: 1 | 2) => {
  ...
  article_store.togglePraise(form, bool => {
    if (type == 1) {
      ...
      if (article.value === null || !article.value.stars) return
...
const scrollView = (name: string, idclass = true) => {
  ...
  let dom: Element
  if (idclass) {
    dom = document.querySelector(`.${name}`) as Element
  } else {
    dom = document.getElementById(name) as Element
  }
  dom.scrollIntoView({ behavior: 'smooth', block: 'center' })
}

const onScroll = (e: Event) => {
  let res = directs.value.find(d => (d.top as number) > window.scrollY)
  if (res) {
    actd.value = res.key as string
  } else {
    actd.value = directs.value[0]
      ? (directs.value[directs.value.length - 1].key as string)
      : ''
  }
  // console.log('滚动：', res)
}
...
onMounted(() => {
  let { id } = route.params
  article_store.getArticlesDetail(id as string, data => {
    article.value = data as ArticleType
    ...
    <div class="main-box fx">
      <div class="content-panel">
        <div class="content" v-if="article">
          ...
          <div className="options fx">
            ...
            <a
              className="edit"
              v-if="user_store.user_state.user_info?._id == article.user._id"
              @click="toEdit"
              >编辑</a
            >
          </div>
      ...
      <div class="other-panel">
        <div class="user-pan pan" v-if="article">
          ...
          <div
            class="btn-wrap fx-b"
            v-if="article.created_by != user_store.user_state.user_info?._id"
          >
